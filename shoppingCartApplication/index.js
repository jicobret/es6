class Cart {
    ...
    addProduct({ name, price, quantity = 1 }) {
        ...
        if (existingProductIndex !== -1) { // sprawdza czy produkt juz istnieje bazujac na wyzszej metodzie findIndex, jezeli tak to zwieksza jego ilosc o podana wartosc
            this.products[existingProductIndex] = {
                ...this.products[existingProductIndex],
                quantity: this.products[existingProductIndex].quantity + quantity
            };
        } else {
            this.products = [...this.products, { name, price, quantity }]; // dodaje produkt jako nowy w przypadku nie wystepowania go w koszyku
        }
